create sequence if not exists public.manual_fixture_id_seq;
create sequence if not exists public.manual_team_id_seq;

alter table if exists public.partidos
  add column if not exists origen_datos text not null default 'manual',
  add column if not exists fuente_detalle text,
  add column if not exists creado_manual_en timestamptz;
...
create or replace function public.predigol_codigo_equipo(p_nombre text)
returns text
language sql
immutable
as $$
      ...
      regexp_replace(
        coalesce(
          nullif(
            array_to_string(
              array(
                select left(palabra, 1)
                from unnest(regexp_split_to_array(trim(coalesce(p_nombre, 'TBD')), '\s+')) as palabras(palabra)
                limit 3
              ),
              ''
            ),
          ...
          left(trim(coalesce(p_nombre, 'TBD')), 3)
        ),
        '[^A-Za-z0-9]',
        '',
        'g'
...
revoke all on function public.predigol_codigo_equipo(text) from public, anon, authenticated;
...
grant execute on function public.predigol_codigo_equipo(text) to service_role;
...
grant execute on function public.cerrar_partido_manual(text, integer, integer) to service_role;
grant execute on function public.cancelar_partido_manual(text) to service_role;
